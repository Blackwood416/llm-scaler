...
// atomic float-bit fetch_max variant triggered UR_RESULT_ERROR_DEVICE_LOST
// on the second H3-shape correctness case, so this path deliberately uses a
// plain scratch + reduce kernel instead of atomics.
        ...
        handle.parallel_for(
            ...
            [=](sycl::nd_item<1> item)
                ...
                const int64_t wg_group =
                    static_cast<int64_t>(item.get_group(0) % wgs);
                const int64_t group = wg_group * groups_per_wg + sub;
