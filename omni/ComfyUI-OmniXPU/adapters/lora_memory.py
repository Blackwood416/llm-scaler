...
def _add_budget_to_load_args(args, kwargs, budget):
    ...
    if "minimum_memory_required" in new_kwargs:
        ...
        # ComfyUI 0.31 placed minimum_memory_required at position 2; newer
        # signatures moved force_patch_weights there and minimum to position
        # 3. Only a real number is a memory size, so bools are skipped.
        for position in (3, 2):
            if len(new_args) <= position:
                continue
            minimum = new_args[position]
            if _is_memory_size(minimum):
                effective_minimum = minimum + budget
                new_args[position] = effective_minimum
                break
